...
async function getOtherNews(currentSlug: string) {
  if (!currentSlug) return [];
  const query = `*[_type == "news" && slug.current != $slug] | order(_createdAt desc)[0...4] {
...
async function getNewsDetail(slug: string) {
  ...
  const query = `*[_type == "news" && (_id == $slug || slug.current == $slug)][0] {
    _id, 
    title, 
...
export default async function NewsDetail(props: any) {
  ...
  const otherNews = await getOtherNews(params.slug);
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-20 px-4 text-center">
          <div className="bg-white p-10 rounded-3xl shadow-xl max-w-lg w-full">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mb-2">Berita Tidak Ditemukan</h2>
            <p className="text-gray-500 mb-8">Mungkin berita yang Anda cari sudah dihapus atau linknya salah.</p>
  ...
  const heroImage = hasImages ? news.images[0] : null;
  const galleryImages = hasImages && news.images.length > 1 ? news.images.slice(1) : [];
      ...
      <section className="relative w-full h-[50vh] md:h-[60vh] min-h-[400px] overflow-hidden bg-green-900">
        ...
        <div className="absolute bottom-0 w-full pb-16 md:pb-24 pt-10">
          <div className="container mx-auto px-4 md:px-8 max-w-5xl">
            ...
            {news.category && (
              <div className="mb-3">
                <span className="bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-md shadow-sm uppercase tracking-wider">
                  {news.category}
                </span>
              </div>
            )}
        ...
        <article className="bg-white rounded-3xl md:rounded-[40px] shadow-2xl p-6 md:p-12 lg:p-16 border border-gray-100">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="prose prose-lg md:prose-xl max-w-none text-gray-700 
              ...
              [&_img]:rounded-2xl [&_img]:shadow-lg [&_img]:my-8 [&_img]:mx-auto [&_img]:w-full [&_img]:object-cover
            ">
              {news.content ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                  ...
                  <p className="text-gray-500 font-medium text-lg">Belum ada isi berita yang ditulis.</p>
                </div>
              )}
            </div>
            ...
            {galleryImages.length > 0 && (
              <div className="mt-12 md:mt-16">
                <h3 className="text-2xl font-bold text-gray-800 mb-6 border-b border-gray-100 pb-4">Galeri Foto</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {galleryImages.map((imgUrl: string, index: number) => (
                    <div key={index} className="relative w-full h-40 md:h-56 rounded-2xl overflow-hidden shadow-md group cursor-pointer">
                      <Image 
                        src={imgUrl} 
                        alt={`${news.title} - foto galeri ${index + 1}`} 
                        fill 
      ...
      {otherNews && otherNews.length > 0 && (
        <section className="container mx-auto px-4 md:px-8 max-w-5xl py-16">
              ...
              return (
                <Link key={item._id} href={targetUrl} className="flex flex-col group cursor-pointer transition-all duration-300 rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-xl border border-gray-100 p-3">
                  <div className="relative w-full h-32 md:h-40 rounded-xl overflow-hidden mb-4 bg-gray-100">
                    ...
                    {item.category && (
                      <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] font-bold px-2 py-1 rounded-md z-10 shadow-sm uppercase tracking-wider">
                        {item.category}
                      </div>
                    )}
                  </div>
          ...
          <div className="mt-12 text-center">
            <Link href="/berita" className="inline-flex items-center justify-center px-8 py-3.5 border-2 border-green-700 text-green-700 hover:bg-green-700 hover:text-white rounded-full font-bold transition-all duration-300 shadow-sm hover:shadow-md group">
              Lihat Semua Berita
