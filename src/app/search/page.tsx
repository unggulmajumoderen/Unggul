              ...
              return (
                <Link key={item._id} href={targetUrl} className="flex flex-col group cursor-pointer transition-all duration-300 rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-xl border border-gray-100 hover:-translate-y-1">
                  <div className="relative w-full overflow-hidden bg-gray-100 flex items-center justify-center">
                    {item.imageUrl ? (
                      <img src={item.imageUrl} alt={item.title} className="w-full h-auto block group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="w-full py-16 flex items-center justify-center text-gray-400 bg-gray-50">
